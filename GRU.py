...
if __name__ == '__main__':
    ...
    a = np.load("Word2vecMIMICmindICD9_8_3times.npy")
    disease_count = 20
    vector = 8
    ...
    for train_index, test_index in kf.split(x):
        ...
        model = Sequential()
        model.add(GRU(100, return_sequences=True, activation="tanh", dropout=0.2))
        model.add(GRU(100, return_sequences=True, activation="tanh", dropout=0.0))
        model.add(GRU(100, return_sequences=True, activation="tanh", dropout=0.0))
        model.add(GRU(100, return_sequences=True, activation="tanh", dropout=0.0))
        model.add(GRU(100, return_sequences=True, activation="tanh", dropout=0.0))
        model.add(GRU(100, return_sequences=True, activation="tanh", dropout=0.0))
