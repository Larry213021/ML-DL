...
if __name__ == '__main__':
    ...
    y = np.vstack((np.repeat(np.array([[1]]), (case_data_length), axis=0),np.repeat(np.array([[0]]), (control_data_length), axis=0)))
    ...
    for train_index, test_index in kf.split(x):
        ...
        model = Sequential()
        model.add(Dense(units=100, activation='relu',input_shape=(disease_count,vector)))
